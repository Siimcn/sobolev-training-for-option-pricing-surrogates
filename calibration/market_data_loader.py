...
class OptionChainError(ValueError):
    """The chain arrived, but nothing usable survived the filters."""
...
class MarketDataLoader:
    ...
    def fetch_yahoo_options(
        ...
        try:
            ...
            return MarketDataLoader._recover_from_failed_fetch(
                error, ticker_symbol, cache_path
            )
    ...
    @staticmethod
    def _recover_from_failed_fetch(
        error: Exception, ticker_symbol: str, cache_path: Optional[str]
    ):

        if cache_path and os.path.exists(cache_path):
            print(
                f"\nWARNING: could not fetch {ticker_symbol} from Yahoo "
                f"({type(error).__name__}: {error}).\n"
                f"Falling back to the cached chain, which may be stale."
            )

            return MarketDataLoader.load_cache(cache_path)

        if isinstance(error, OptionChainError):
            raise error

        raise ConnectionError(
            f"Could not load the {ticker_symbol} option chain from Yahoo "
            f"({type(error).__name__}: {error}), and no cached chain exists "
            f"at {cache_path or '<no cache_path configured>'}. Check the "
            f"ticker symbol and the network connection; a single run with a "
            f"working connection writes the cache that later offline runs "
            f"reuse."
        ) from error
    ...
    def _fetch_from_yahoo(
        ...
        if not expirations:
            raise OptionChainError(f"No option expiries found for " f"{ticker_symbol}.")
