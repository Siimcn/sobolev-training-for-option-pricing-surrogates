...
def mean_relative_error(
    ...
    scale_floor = jnp.maximum(
        eps,
        1e-3 * jnp.mean(jnp.abs(target)),
    )
...
def price_metrics(
    ...
    return {
        "RMSE": float(
            rmse(prices_pred, prices_true)
        ),
        "MAE": float(
            mae(prices_pred, prices_true)
        ),
        "Relative_L2": float(
            relative_l2_error(
                prices_pred,
...
def sobolev_metrics(
    ...
    if (
        gradients_pred is not None
        and gradients_true is not None
    ):
        metrics.update(
            gradient_metrics(
                gradients_pred,
    ...
    if (
        hvps_pred is not None
        and hvps_true is not None
    ):
        metrics.update(
            hvp_metrics(
                hvps_pred,
