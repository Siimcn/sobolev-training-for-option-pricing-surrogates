...
class TrainingConfig:
    ...
    def validate(self) -> None:
        ...
        if self.sobolev_order not in (0, 1, 2):
            raise ValueError("sobolev_order must be 0, 1 or 2.")

        if self.lambda_grad < 0:
            raise ValueError("lambda_grad must be non-negative.")

        if self.lambda_hessian < 0:
            raise ValueError("lambda_hessian must be non-negative.")

        if self.min_delta_relative < 0:
            raise ValueError("min_delta_relative must be non-negative.")

        if self.selection_metric not in SELECTION_METRICS:
            raise ValueError(
                f"selection_metric must be one of: " f"{', '.join(SELECTION_METRICS)}."
            )

        if self.lr_schedule not in LR_SCHEDULES:
            raise ValueError(f"lr_schedule must be one of: {', '.join(LR_SCHEDULES)}.")

        if not 0.0 < self.lr_final_fraction <= 1.0:
            raise ValueError("lr_final_fraction must lie in (0, 1].")

        if self.warmup_epochs < 0:
            raise ValueError("warmup_epochs must be non-negative.")

        if self.gradient_clip is not None and self.gradient_clip <= 0:
            raise ValueError("gradient_clip must be positive, or None to disable it.")
