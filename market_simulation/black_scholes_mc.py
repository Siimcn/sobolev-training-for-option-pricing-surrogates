...
from market_simulation.pricing_model import BlackScholesParams, BlackScholesModel

from market_simulation.timesteppingscheme import EulerMaruyama

from market_simulation.payoff import payoff_spec, sigmoid_smooth

from market_simulation.monte_carlo_pricer import MonteCarloPricer

from market_simulation.sobolev_labels import create_sobolev_labels
...
def make_mc_calibration_pricer(
    ...
    groups = [
        (float(T), jnp.asarray(np.where(maturities == T)[0]))
    ...
    def mc_pricing_fn(params, strikes, maturities, is_call):

        prices = jnp.zeros(num_instruments)

        for group_index, (T, indices) in enumerate(groups):

            key = jax.random.fold_in(base_key, group_index)
            ...
            omega = jnp.where(is_call[indices], 1.0, -1.0)

            intrinsic = omega[None, :] * (terminal[:, None] - strikes[indices][None, :])

            payoffs = sigmoid_smooth(
                intrinsic, _payoff_smoothing_width(spot, T, params.sigma)
            )

            discount = jnp.exp(-params.r * T)

            prices = prices.at[indices].set(discount * jnp.mean(payoffs, axis=0))
...
def generate_training_paths(x: jnp.ndarray, num_paths: int = 100):
